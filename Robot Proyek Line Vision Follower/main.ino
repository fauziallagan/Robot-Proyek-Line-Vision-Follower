/*
  ...
  Servo -> IO2 (Motor servo dan catu daya eksternal ground bersama ESP32-CAM)
  ...
  http://192.168.1.100/control?ip                      //Dapatkan Access Point IP, Station IP
  http://192.168.1.100/control?mac                     //Dapatkan Alamat MAC
  http://192.168.1.100/control?restart                 //Reboot ESP32-CAM
  http://192.168.1.100/control?digitalwrite=pin;value  //Hasil Digital
  http://192.168.1.100/control?analogwrite=pin;value   //Hasil Analog
  http://192.168.1.100/control?digitalread=pin         //Membaca Digital
  http://192.168.1.100/control?analogread=pin          //Membaca Analog
  http://192.168.1.100/control?touchread=pin           //Sentuh untuk Membaca
  http://192.168.1.100/control?resetwifi=ssid;password   //Setel ulang jaringan Wifi
  ...
  http://192.168.1.100/control?var=hmirror&val=value      // value = 0 or 1
  http://192.168.1.100/control?var=vflip&val=value        // value = 0 or 1
  http://192.168.1.100/control?var=flash&val=value        // value = 0 ~ 255
*/
...
// masukkan SSID dan Password untuk Akses Point Internal  http://192.168.4.1
const char *apssid = "<SSID_BUAT_ACCESS_POINT>";
const char *appassword = "<PASSWORD_BUAT_ACCESS_POINT>"; // Note : Minimal 8 karakter atau lebih

int pinServo = 2;       // pin servo
int servoAngle = 30;    // sudut awal servo
...
#include <WiFi.h>             //library yang digunakan untuk mengakses WiFi
#include "soc/soc.h"          // library yang digunakan untuk menonaktifkan masalah / catu daya yang tidak stabil
#include "soc/rtc_cntl_reg.h" //library yang digunakan untuk menonaktifkan masalah
#include <esp32-hal-ledc.h>   // library untuk mengendalikan servo / kontrol servo
...
String Feedback = ""; // variabel untuk menangani perintah khusus,atau untuk mengembalikan pesan kepada client

// variabel untuk parameter khusus
String Command = "";
String cmd = "";
String P1 = "";
String P2 = "";
...
// Nilai status pembongkaran perintah khusus
byte ReceiveState = 0;
byte cmdState = 1;
byte strState = 1;
...
void setup()
{
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0); // Pengaturan restart ketika daya dimatikan tidak stabil

  Serial.begin(115200);
  Serial.setDebugOutput(true); // Aktifkan keluaran debug
  Serial.println();

  // pengaturan konfigurasi video/cam  https://github.com/espressif/esp32-camera/blob/master/driver/include/esp_camera.h
  ...
  if (psramFound())
  { // pengecekan IC memory PSRAM (Pseudo SRAM)
  ...
  // Ukuran default bingkai video yang dapat disesuaikan (ukuran resolusi)
  ...
  // Mengubah ukuran bingkai video secara dinamis (ukuran resolusi)
  s->set_framesize(s, FRAMESIZE_QVGA);

  // Cermin
  //  s->set_hmirror(s, 1);
  //  s->set_vflip(s, 1);  //Membalik secara Vertikal
  ...
  WiFi.mode(WIFI_AP_STA); // mode lainnya WiFi.mode(WIFI_AP); WiFi.mode(WIFI_STA);

  // Menentukan IP statis klien
  //  WiFi.config(IPAddress(192, 168, 201, 100), IPAddress(192, 168, 201, 2), IPAddress(255, 255, 255, 0));

  for (int i = 0; i < 2; i++)
  {
    WiFi.begin(ssid, password); // koneksi jaringan
    ...
    while (WiFi.status() != WL_CONNECTED)
    {
      ...
      if ((StartTime + 5000) < millis())
        break; // waktu tunggu dalam millis();
    }

    if (WiFi.status() == WL_CONNECTED)
    {                                                                                      // connection successfully
      WiFi.softAP((WiFi.localIP().toString() + "_" + (String)apssid).c_str(), appassword); // set ssid AP
      Serial.println("");
      Serial.println("STAIP address: ");
      Serial.println(WiFi.localIP()); // menampilkan IP access Point
      Serial.println("");

      for (int i = 0; i < 5; i++)
      { // flash akan berkedip bila wifi sudah terhubung
        ledcWrite(4, 10);
        delay(200);
        ledcWrite(4, 0);
        delay(200);
  ...
  { // jika koneksi gagal
    ...
    for (int i = 0; i < 2; i++)
    { // flash akan berkedip bila tidak terhubung ke wifi
      ledcWrite(4, 10);
      delay(1000);
      ledcWrite(4, 0);
      delay(1000);
  ...
  // Menentukan IP address pada Access Point
  ...
  // Mengatur lampu flash ke level rendah
  pinMode(4, OUTPUT);
  digitalWrite(4, LOW);
}
...
{
  char *buf; // string paramater setelah url diakses
  size_t buf_len;
  char variable[128] = {
      0,
  }; // Akses Parameter Nilai variabel
  char value[128] = {
      0,
  }; // Mengakses nilai paramater
  String myCmd = "";
  ...
  if (buf_len > 1)
  {
    ...
    if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK)
    {
      ...
      {
        myCmd = String(buf); //, val，format perintah khusus
      }
    }
  }
  else
  ...
  if (myCmd.length() > 0)
  {
    myCmd = "?" + myCmd; // String parameter setelah URL diubah menjadi format perintah khusus
    for (int i = 0; i < myCmd.length(); i++)
    {
      getCommand(char(myCmd.charAt(i))); // Buka paket string parameter perintah khusus
    }
  }
  ...
  if (cmd.length() > 0)
  {
    ...
    // perintah khusus. Contoh :   http://192.168.1.100/control?cmd=P1;P2;P3;P4;P5;P6;P7;P8;P9
    if (cmd == "your cmd")
    {
      // You can do anything
      // Feedback="<font color=\"red\">Hello World</font>";   //Dapat berupa teks normal atau sintaks HTML
    }
    else if (cmd == "ip")
    { //查詢APIP, STAIP
    ...
    { // atur ulang koneksi jaringan  //note
      for (int i = 0; i < 2; i++)
      {
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
          ...
          for (int i = 0; i < 2; i++)
          { // kondisi jika tidak terhubung ke wifi, flash akan berkedip perlahan
            ledcWrite(4, 10);
            delay(300);
            ledcWrite(4, 0);
            delay(300);
    ...
    { // keadaan gerak roda
      int val = P1.toInt();
      if (val == 1)
      { // Depan http://192.168.1.100/control?car=1
      ...
      { // Kiri http://192.168.1.100/control?car=2
        Serial.println("Left");
      ...
      { // Berhenti http://192.168.1.100/control?car=3
        Serial.println("Stop");
        ledcWrite(5, 0);
        ledcWrite(6, 0);
        ledcWrite(7, 0);
      ...
      { // Kanan http://192.168.1.100/control?car=4
      ...
      { // belakang http://192.168.1.100/control?car=5
        Serial.println("Back");
        ledcWrite(5, 0);
        ledcWrite(6, speedR);
        ledcWrite(7, 0);
      ...
      { // kiri depan http://192.168.1.100/control?car=6
      ...
      { // kanan depan http://192.168.1.100/control?car=7
      ...
      { // belakang kiri http://192.168.1.100/control?car=8
      ...
      { // belakang kanan  http://192.168.1.100/control?car=9
    ...
    if (Feedback == "")
      Feedback = Command; // jika tidak ada pengembalian, kembalikan perintah variabel feedback

    const char *resp = Feedback.c_str();
    httpd_resp_set_type(req, "text/html");                       // format return
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*"); // Izin pembacaan domain
    return httpd_resp_send(req, resp, strlen(resp));
  }
  else
  {
    // parameter resmi dengan meakses url berikut  http://192.168.1.100/control?var=xxx&val=xxx
    int val = atoi(value);
    sensor_t *s = esp_camera_sensor_get();
    int res = 0;
    ...
    { // Keadaan Gerak
      ...
      { // kembali http://192.168.1.100control?car=5
        Serial.println("Back");
        ledcWrite(5, 0);
        ledcWrite(6, speedR);
        ledcWrite(7, 0);
      ...
      { // Maju Kanan http://192.168.1.100/control?car=7
        Serial.println("FrontRight");
        ledcWrite(5, speedR * decelerate);
        ledcWrite(6, 0);
        ledcWrite(7, speedL);
    ...
    if (buf)
    {
      ...
      httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
      return httpd_resp_send(req, resp, strlen(resp)); // mengembalikan string parameter
    }
    else
    {
      httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
...
// Tampilkan status parameter video (harus mengembalikan format json untuk memuat pengaturan awal)
...
<head>
    ...
    <figure>
      ...
      <section id="buttons">
        ...
        <script> 
          //  URL/?192.168.1.38  Secara otomatis dapat membawa nilai IP dari parameter terakhir
          ...
          function start() {
            ...
            //Menambahkan acara klik tombol daya restart (format perintah khusus)：http://192.168.1.100/control?cmd=P1;P2;P3;P4;P5;P6;P7;P8;P9)
            ...
            // kategori khusus my-action, title nilai tampilan properti
            document
          ...
          const turnDelayMax = document.getElementById('turnDelayMax');  //Waktu rotasi saat bergerak menjauh
          const turnDelayMin = document.getElementById('turnDelayMin');  //Waktu slewing pada deviasi
          const forwardDelay = document.getElementById('forwardDelay');  //durasi saat maju
          var servoAngle = 30;  //Posisi preset motor servo
          var lastDirection = "";  //Catat arah perjalanan aksi sebelumnya
        ...
        tracker.on('track', function(event) {
          ...
          if (rect.color=="red"&&motorState.checked) {
            ...
            if (xls==0&&xrs==0) {
              ...
              result.innerHTML= "paling kanan";
              position = "rightfar";
            } else {
              result.innerHTML= "tidak bisa dibedakan";
              position = "";
            }
            ...
            if (position.indexOf("right")!=-1) {
              ...
              } else {  //Titik pusat objek jauh dari kiri
              ...
              if (position=="left") { //Titik pusat objek ke kanan
                delay = turnDelayMin.value;
              } else { //Titik pusat benda jauh dari kanan
                delay = turnDelayMax.value;
              }
              if (!hmirror.checked) {  //Posisi spion kiri dan kanan berlawanan
                car('/control?car=7;'+delay);  //maju ke kanan
            ...
            } else if (position=="normal") {  //Majulah jika Anda berada di garis
        ...
        if (event.data.length==0&&motorState.checked) {  //Jika tidak ada garis yang terdeteksi di layar sama sekali, putar ke arah sebaliknya sesuai dengan tindakan sebelumnya。
...
void startCameraServer()
{
  ...
  {
    // Daftarkan fungsi yang akan dijalankan sesuai dengan jalur URL khusus
...
// Perintah khusus membongkar string parameter ke dalam variabel
