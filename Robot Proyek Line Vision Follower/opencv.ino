/*
  ...
  http://192.168.xxx.xxx/control?ip                      //Dapatkan Access Point IP, Station IP
  http://192.168.xxx.xxx/control?mac                     //Dapatkan Alamat MAC
  http://192.168.xxx.xxx/control?restart                 //Reboot ESP32-CAM
  http://192.168.xxx.xxx/control?digitalwrite=pin;value  //Hasil Digital
  http://192.168.xxx.xxx/control?analogwrite=pin;value   //Hasil Analog
  http://192.168.xxx.xxx/control?digitalread=pin         //Membaca Digital
  http://192.168.xxx.xxx/control?analogread=pin          //Membaca Analog
  http://192.168.xxx.xxx/control?touchread=pin           //Sentuh untuk Membaca
  http://192.168.xxx.xxx/control?resetwifi=ssid;password   //Setel ulang jaringan Wifi
  http://192.168.xxx.xxx/control?flash=value               //Flash value= 0-255
  http://192.168.xxx.xxx/control?servo=value               //Motor Servo value = 0-180
...
const char *apssid = "esp32-tes";
const char *appassword = "123456789"; // Note : Minimal 8 karakter atau lebih
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
  ...
  if (cmd.length() > 0)
  {
    ...
    // perintah khusus  http://192.168.xxx.xxx/control?cmd=P1;P2;P3;P4;P5;P6;P7;P8;P9
    if (cmd == "your cmd")
    {
      // You can do anything
    ...
    { // keadaan gerak roda
      int val = P1.toInt();
      if (val == 1)
      { // Depan http://192.168.xxx.xxx/control?car=1
      ...
      { // Kiri http://192.168.xxx.xxx/control?car=2
        Serial.println("Left");
      ...
      { // Berhenti http://192.168.xxx.xxx/control?car=3
        Serial.println("Stop");
        ledcWrite(5, 0);
        ledcWrite(6, 0);
        ledcWrite(7, 0);
      ...
      { // Kanan http://192.168.xxx.xxx/control?car=4
      ...
      { // belakang http://192.168.xxx.xxx/control?car=5
        Serial.println("Back");
        ledcWrite(5, 0);
        ledcWrite(6, speedR);
        ledcWrite(7, 0);
      ...
      { // kiri depan http://192.168.xxx.xxx/control?car=6
      ...
      { // kanan depan http://192.168.xxx.xxx/control?car=7
      ...
      { // belakang kiri http://192.168.xxx.xxx/control?car=8
      ...
      { // belakang kanan  http://192.168.xxx.xxx/control?car=9
  ...
  {
    // parameter resmi dengan meakses url berikut  http://192.168.xxx.xxx/control?var=xxx&val=xxx
    int val = atoi(value);
    sensor_t *s = esp_camera_sensor_get();
    int res = 0;
    ...
    { // Keadaan Gerak
      ...
      { // kembali http://192.168.xxx.xxx/control?car=5
        Serial.println("Back");
        ledcWrite(5, 0);
        ledcWrite(6, speedR);
        ledcWrite(7, 0);
      ...
      { // Maju Kanan http://192.168.xxx.xxx/control?car=7
        Serial.println("FrontRight");
        ledcWrite(5, speedR * decelerate);
        ledcWrite(6, 0);
        ledcWrite(7, speedL);
      ...
      <section id="buttons">
        ...
        <script> 
          ...
          function start() {
            ...
            //Menambahkan acara klik tombol daya restart (format perintah khusus)：http://192.168.xxx.xxx/control?cmd=P1;P2;P3;P4;P5;P6;P7;P8;P9)
